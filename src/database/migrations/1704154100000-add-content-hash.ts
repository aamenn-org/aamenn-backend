...
export class AddContentHash1704154100000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Add content_hash column for duplicate detection (if not exists)
    await queryRunner.query(`
      ALTER TABLE "files" 
      ADD COLUMN IF NOT EXISTS "content_hash" varchar(64)
    `);

    // Create index for fast duplicate lookups (if not exists)
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_files_content_hash" ON "files" ("content_hash")
      WHERE "content_hash" IS NOT NULL
    `);

    // Create compound index for user + hash lookups (if not exists)
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_files_user_content_hash" ON "files" ("user_id", "content_hash")
