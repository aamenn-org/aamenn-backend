...
export class InstapayService {
  ...
  private qrCache: { username: string; dataUrl: string } | null = null;
  ...
  async getInfo(): Promise<InstapayInfo> {
    ...
    const qrDataUrl = username ? await this.getQrDataUrl(username) : null;

    return { enabled, username, qrDataUrl };
  }

  private async getQrDataUrl(username: string): Promise<string> {
    if (this.qrCache && this.qrCache.username === username) {
      return this.qrCache.dataUrl;
    }
    const dataUrl = await QRCode.toDataURL(username, {
      errorCorrectionLevel: 'M',
      margin: 2,
      width: 320,
    });
    this.qrCache = { username, dataUrl };
    return dataUrl;
  }
